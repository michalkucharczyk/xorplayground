...
fn main() {
    ...
    let decoded = vec![&decoded0, &decoded1, &decoded2, &decoded3, &decoded4, &decoded5, &decoded6, &decoded7 ];
    ...
    for d in decoded.clone() {
        ...
        let xored = decoded0.iter().zip(d).map(|(x,y)| {x ^ y}).collect::<Vec<_>>();
        let xored = xored.iter().skip(0).zip(&p0).map(|(x,y)| {x ^ y}).collect::<Vec<_>>();
        let other_plaintext_v = xored.clone();
        let other_plaintext = str::from_utf8(&xored);

        println!("other:{other_plaintext:?}");

        let xored = d.iter().zip(other_plaintext_v).map(|(x,y)| {x ^ y}).collect::<Vec<_>>();
